-- Fix security warnings and function issues

-- 1. Drop the problematic view that exposes auth.users
DROP VIEW IF EXISTS public.user_roles_view;

-- 2. Drop and recreate the function with proper signature
DROP FUNCTION IF EXISTS public.get_user_role_info(uuid);

-- 3. Create a secure function to get user role information
...
BEGIN
  ...
  SELECT 
    ...
    p.email::text,
    p.role::app_role as primary_role,
...
-- 4. Update all existing functions to have proper search_path (no changes to signature)
