...
-- Create RLS policies (removing IF NOT EXISTS which causes syntax error)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'compliance_suppression_list' 
    AND policyname = 'Authenticated users can manage suppression list'
  ) THEN
    CREATE POLICY "Authenticated users can manage suppression list" 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'lead_consent_audit' 
    AND policyname = 'Authenticated users can manage consent audit'
  ) THEN
    CREATE POLICY "Authenticated users can manage consent audit" 
