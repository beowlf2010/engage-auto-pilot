...
-- Create a completely new function with a different name to bypass caching
DROP FUNCTION IF EXISTS public.upload_csv_leads_v2(jsonb, uuid) CASCADE;

CREATE OR REPLACE FUNCTION public.upload_csv_leads_v2(
  p_leads jsonb,
...
BEGIN
  -- Add logging to verify this is the new cache-busting function
  RAISE NOTICE 'CACHE-BUSTING FUNCTION V2 CALLED: Processing % leads', jsonb_array_length(p_leads);
  ...
  result := jsonb_build_object(
    ...
    'message', format('CACHE-BUSTING V2: Processed %s leads, %s successful inserts, %s errors', 
...
EXCEPTION WHEN OTHERS THEN
  ...
  RETURN jsonb_build_object(
    ...
    'errors', jsonb_build_array(
      jsonb_build_object(
        'error', 'CACHE-BUSTING V2 execution failed: ' || SQLERRM,
        'sqlstate', SQLSTATE,
        'timestamp', now()
      )
    ),
    'message', 'Cache-busting V2 function failed with critical error'
  );
END;
$$;

-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION public.upload_csv_leads_v2(jsonb, uuid) TO authenticated;

-- Add comment to verify this is the cache-busting function
COMMENT ON FUNCTION public.upload_csv_leads_v2(jsonb, uuid) IS 'Cache-busting bypass function without session_replication_role - v2';
