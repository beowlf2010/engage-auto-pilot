-- Fix security warnings - handle function signature properly

-- 1. Drop the problematic view that exposes auth.users
DROP VIEW IF EXISTS public.user_roles_view;

-- 2. Drop existing function if it exists with different signature
...
-- 4. Update all existing functions to have proper search_path (fixed versions)
