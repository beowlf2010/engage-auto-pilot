-- Update purge_old_leads function to handle all required columns
CREATE OR REPLACE FUNCTION public.purge_old_leads(p_cutoff timestamp with time zone DEFAULT (now() - '90 days'::interval), p_dry_run boolean DEFAULT true)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF anon_id IS NULL THEN
    INSERT INTO public.leads (
      ...
      'anonymized_system', 
      'archived',
      'General inquiry - anonymized for data retention',
      now(), 
      now()
