...
-- 1. DISABLE ALL CRON JOBS FOR AI AUTOMATION (with error handling)
DO $$
BEGIN
    -- Try to unschedule each job, ignore errors if job doesn't exist
    BEGIN
        PERFORM cron.unschedule('ai-automation-robust');
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Ignore if job doesn't exist
    END;
    
    BEGIN
        PERFORM cron.unschedule('ai-automation-every-10-minutes');
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Ignore if job doesn't exist
    END;
    
    BEGIN
        PERFORM cron.unschedule('ai-automation-scheduler');
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Ignore if job doesn't exist
    END;
    
    BEGIN
        PERFORM cron.unschedule('ai-automation-every-15-minutes');
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Ignore if job doesn't exist
    END;
    
    BEGIN
        PERFORM cron.unschedule('enhanced-ai-automation-peak-hours');
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Ignore if job doesn't exist
    END;
    
    BEGIN
        PERFORM cron.unschedule('enhanced-ai-automation-off-peak');
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Ignore if job doesn't exist
    END;
    
    BEGIN
        PERFORM cron.unschedule('ai-queue-health-monitor');
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Ignore if job doesn't exist
    END;
END $$;
...
INSERT INTO public.ai_emergency_settings (ai_disabled, disable_reason, disabled_at)
SELECT true, 'EMERGENCY SHUTDOWN - Messages still being sent despite previous emergency stop', now()
...
INSERT INTO public.ai_automation_runs (
  ...
  now(),
  now(),
  'EMERGENCY SHUTDOWN: Messages still being sent despite previous emergency stop - immediate halt executed',
  0,
  0,
  0
);
