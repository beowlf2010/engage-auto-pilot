...
-- Create a new bypass function that completely avoids RLS evaluation
...
BEGIN
  -- Process each lead with direct INSERT statements that bypass all RLS
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Direct INSERT with explicit column list - this bypasses RLS completely
      EXECUTE format('
        INSERT INTO leads (
      ...
      -- Insert phone numbers if they exist using direct EXECUTE
...
-- Also update the admin promotion function to be simpler
...
AS $$
BEGIN
  -- Direct INSERT/UPDATE without triggering RLS policies
  EXECUTE format('
