-- Secure conversation_summaries policies (retry with string EXECUTE)
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Managers can manage conversation_summaries" '
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Users can view conversation_summaries for their leads" '
         || 'ON public.conversation_summaries '
         || 'FOR SELECT '
         || 'USING (EXISTS (SELECT 1 FROM public.leads l WHERE l.id = conversation_summaries.lead_id '
