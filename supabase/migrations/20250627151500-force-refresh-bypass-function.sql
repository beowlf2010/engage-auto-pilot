...
-- Force drop and recreate the bypass function to clear any cache issues
DROP FUNCTION IF EXISTS public.upload_csv_leads_bypass_rls(jsonb, uuid) CASCADE;

-- Create the ultimate Supabase-compatible bypass function (force refresh)
...
BEGIN
  -- Add logging to verify this is the new function
  RAISE NOTICE 'NEW BYPASS FUNCTION CALLED: Processing % leads', jsonb_array_length(p_leads);
  ...
  result := jsonb_build_object(
    ...
    'message', format('NEW FUNCTION: Processed %s leads, %s successful inserts, %s errors', 
...
EXCEPTION WHEN OTHERS THEN
  ...
  RETURN jsonb_build_object(
    ...
    'errors', jsonb_build_array(
      jsonb_build_object(
        'error', 'NEW FUNCTION execution failed: ' || SQLERRM,
        'sqlstate', SQLSTATE,
        'timestamp', now()
      )
    ),
    'message', 'NEW FUNCTION failed with critical error'
  );
END;
$$;
...
-- Add comment to verify this is the new function
COMMENT ON FUNCTION public.upload_csv_leads_bypass_rls(jsonb, uuid) IS 'Updated bypass function without session_replication_role - v2';
