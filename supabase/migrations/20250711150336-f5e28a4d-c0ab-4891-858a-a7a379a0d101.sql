-- Drop the existing function and recreate with better authentication handling
DROP FUNCTION IF EXISTS public.insert_inventory_secure(jsonb, uuid);

-- Create improved version that handles RPC authentication properly
CREATE OR REPLACE FUNCTION public.insert_inventory_secure(
  p_vehicles jsonb,
  p_upload_history_id uuid
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  vehicle_record jsonb;
  inserted_count integer := 0;
  total_count integer := 0;
  error_details jsonb := '[]'::jsonb;
  current_user_id uuid;
  user_has_permission boolean := false;
  result jsonb;
BEGIN
  -- Get current user ID
  current_user_id := auth.uid();
  
  -- Log authentication debug info
  RAISE NOTICE 'Authentication Debug - User ID: %, Session: %', 
    current_user_id, 
    COALESCE(auth.jwt()::text, 'No JWT found');
  
  -- Check if user is authenticated
  IF current_user_id IS NULL THEN
    -- Try alternative authentication check
    IF auth.jwt() IS NOT NULL THEN
      -- Extract user ID from JWT if available
      current_user_id := (auth.jwt() ->> 'sub')::uuid;
      RAISE NOTICE 'Extracted user ID from JWT: %', current_user_id;
    END IF;
    
    IF current_user_id IS NULL THEN
      RETURN jsonb_build_object(
        'success', false,
        'inserted_count', 0,
        'total_count', 0,
        'error', 'User authentication required for inventory upload',
        'error_details', '[]'::jsonb
      );
    END IF;
  END IF;
  
  -- Check user permissions with better error handling
  BEGIN
    SELECT EXISTS (
      SELECT 1 FROM public.user_roles 
      WHERE user_id = current_user_id 
      AND role IN ('admin', 'manager', 'sales')
    ) INTO user_has_permission;
    
    RAISE NOTICE 'Permission check result: % for user: %', user_has_permission, current_user_id;
    
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Permission check failed: %', SQLERRM;
    RETURN jsonb_build_object(
      'success', false,
      'inserted_count', 0,
      'total_count', 0,
      'error', 'Permission verification failed: ' || SQLERRM,
      'error_details', '[]'::jsonb
    );
  END;
  
  IF NOT user_has_permission THEN
    RETURN jsonb_build_object(
      'success', false,
      'inserted_count', 0,
      'total_count', 0,
      'error', 'Insufficient permissions: Manager, admin, or sales role required',
      'error_details', '[]'::jsonb
    );
  END IF;
  
  -- Process vehicles
  total_count := jsonb_array_length(p_vehicles);
  
  FOR vehicle_record IN SELECT * FROM jsonb_array_elements(p_vehicles)
  LOOP
    BEGIN
      -- Insert vehicle with proper error handling
      INSERT INTO public.inventory (
        id, make, model, year, vin, stock_number, price, mileage, condition,
        exterior_color, interior_color, transmission, drivetrain, fuel_type,
        body_style, engine, trim_level, status, upload_history_id,
        created_at, updated_at
      ) VALUES (
        COALESCE((vehicle_record->>'id')::uuid, gen_random_uuid()),
        vehicle_record->>'make',
        vehicle_record->>'model',
        COALESCE((vehicle_record->>'year')::integer, 0),
        vehicle_record->>'vin',
        vehicle_record->>'stock_number',
        COALESCE((vehicle_record->>'price')::numeric, 0),
        COALESCE((vehicle_record->>'mileage')::integer, 0),
        COALESCE(vehicle_record->>'condition', 'used'),
        vehicle_record->>'exterior_color',
        vehicle_record->>'interior_color',
        vehicle_record->>'transmission',
        vehicle_record->>'drivetrain',
        vehicle_record->>'fuel_type',
        vehicle_record->>'body_style',
        vehicle_record->>'engine',
        vehicle_record->>'trim_level',
        COALESCE(vehicle_record->>'status', 'available'),
        p_upload_history_id,
        now(),
        now()
      );
      
      inserted_count := inserted_count + 1;
      
    EXCEPTION WHEN OTHERS THEN
      error_details := error_details || jsonb_build_object(
        'vehicle_index', total_count - jsonb_array_length(p_vehicles) + 1,
        'error', SQLERRM,
        'sqlstate', SQLSTATE,
        'vehicle_data', vehicle_record
      );
    END;
  END LOOP;
  
  -- Return comprehensive result
  result := jsonb_build_object(
    'success', inserted_count > 0,
    'inserted_count', inserted_count,
    'total_count', total_count,
    'error_details', error_details
  );
  
  IF inserted_count = 0 AND total_count > 0 THEN
    result := result || jsonb_build_object('error', 'No vehicles could be inserted');
  END IF;
  
  RAISE NOTICE 'Function result: %', result;
  
  RETURN result;
END;
$$;