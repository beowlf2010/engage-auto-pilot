...
const DataQualityCard: React.FC<DataQualityCardProps> = ({
  ...
  // Add null checking for all data accesses
  if (!displayDataQuality || !debugInfo) {
    console.log('DataQualityCard: Missing required data', { displayDataQuality, debugInfo });
    return null;
  }

  // Additional null checking for nested properties
  if (!debugInfo.dataQuality && !debugInfo.originalDataQuality) {
    console.log('DataQualityCard: Missing dataQuality in debugInfo', debugInfo);
    return null;
  }

  // Use fallback data if needed
  const originalDataQuality = debugInfo.dataQuality || debugInfo.originalDataQuality;
  if (!originalDataQuality) {
    console.log('DataQualityCard: No original data quality found');
    return null;
  }
    ...
    <Card className="bg-gray-50 border-gray-200">
      <CardContent className="p-3">
        <div className="text-xs space-y-2">
          ...
          {displayDataQuality.overallQualityScore !== undefined && (
          ...
          {/* Message Strategy */}
          {displayDataQuality.messageStrategy && (
            <div className="flex items-center justify-between">
          ...
          {displayDataQuality.nameValidation && (
            <div className="border-t pt-2">
              ...
              {debugInfo.originalFirstName && originalDataQuality.nameValidation && (
                <div className="text-gray-600 pl-5 mb-2">
                  "{debugInfo.originalFirstName}" detected as: <span className="font-medium">{originalDataQuality.nameValidation.detectedType}</span>
                  {' '}({Math.round(originalDataQuality.nameValidation.confidence * 100)}% confidence)
                </div>
              )}
              {!displayDataQuality.nameValidation.isValidPersonalName && (
                <Button
          ...
          {displayDataQuality.vehicleValidation && (
            <div className="border-t pt-2">
              <div className="flex items-center gap-2 mb-1">
              ...
              {originalDataQuality.vehicleValidation && (
                <div className="text-gray-600 pl-5 mb-2">
                  "{debugInfo.originalVehicleInterest || 'Not specified'}" - {originalDataQuality.vehicleValidation.detectedIssue}
                  {' '}({Math.round(originalDataQuality.vehicleValidation.confidence * 100)}% confidence)
                </div>
              )}
              {originalDataQuality.vehicleValidation?.detectedIssue === 'corruption' && (
